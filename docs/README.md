...
> X Account Cleaner 是一个本地优先的开源 X / Twitter 账号清理 CLI，基于 Node.js、TypeScript、Playwright 和 Chromium。它适合用户清理自己账号的推文、回复、转推、点赞、书签，并用人工确认流程管理关注取关；它不调用官方 X API，也不承诺规避账号风控。

## 30 秒阅读路线

| 如果你是 | 先读 | 目的 |
|---|---|---|
| 第一次进入仓库的新用户 | [../README.md](../README.md) | 判断项目是否适合、了解能力边界和风险 |
| 只想快速运行一次 | [../QUICKSTART.md](../QUICKSTART.md) | 安装依赖、复制配置、运行 `--help` 和小批量试跑 |
| 只想清理关注列表 | [FOLLOWING_MANAGEMENT.md](FOLLOWING_MANAGEMENT.md) | 走只读导出、规则筛选、人工确认、dry-run、execute |
| 遇到页面按钮找不到 | [SELECTOR_UPDATE_GUIDE.md](SELECTOR_UPDATE_GUIDE.md) | 更新 `selectors.json` 并验证选择器 |
| 维护者或贡献者 | [ARCHITECTURE.md](ARCHITECTURE.md) | 理解 CLI、浏览器、登录、删除器和关注管理模块 |
...
- 发现功能失效或有改进想法，欢迎[提交 Issue](https://github.com/tytsxai/x-account-cleaner/issues) 或 PR，验证过的新选择器是最受欢迎的贡献。

## 最短可执行路径

当前唯一可用的安装方式是从源码运行（npm 包尚未发布）：
...
- 默认访问 `https://x.com`（`twitter.com` 会 301 跳转），只做网页 UI 自动化，不调用官方 API，也不使用私有 GraphQL 接口。
- X 页面结构随时可能变化，选择器维护是项目长期成本之一；选择器集中在 `selectors.json`，改配置不用改代码。
